import pandas as pd
from sqlalchemy import create_engine, text
from dotenv import load_dotenv
import os
from tkinter import filedialog, messagebox 
import gui
import sys

def database_connection():
    #loading database info to variables from the .env file
    load_dotenv()
    db_host = os.getenv('POSTGRES_HOST')
    db_port = os.getenv('POSTGRES_PORT')
    db_user = os.getenv('POSTGRES_USER')
    db_password = os.getenv('POSTGRES_PASSWORD')
    db_name = os.getenv('POSTGRES_DB')

    #passing all db info to one string variable
    connection_string = f'postgresql+psycopg2://{db_user}:{db_password}@{db_host}:{db_port}/{db_name}'
    engine = create_engine(connection_string)
    return engine

#function that opens a file explorer window to choose a file to work on
def file_select():
    while True:
        file_path = filedialog.askopenfilename()
        if os.path.splitext(file_path)[1] != '.csv':
            messagebox.showerror('Wrong file format', 'The file you select must be a csv file.')
            if messagebox.askretrycancel('Retry', 'Would you like to select another file?') == False:
                sys.exit()
        else:
            messagebox.showinfo('Correct', 'File type correct.')
            break
    return file_path
     
#function that loads data from a csv file to database
def load_data(file_path, connection, selected_year, selected_quarter):
        df = pd.read_csv(file_path)
        df['year'] = selected_year
        df['quarter'] = selected_quarter
        table_name = f'raw_{selected_quarter.lower()}_{selected_year}'
        df.to_sql(table_name, con=connection, if_exists='replace', index=False)
        connection.commit()
        messagebox.showinfo('Correct', f'{file_path} loaded to {table_name} table.')

def move_data_to_cleaned_tables(connection):
    with open('scripts/transform_load.sql', 'r') as file:
        print('File opened successfully.')
        sql_script = file.read()
        try:
            print('Executing SQL script...')
            connection.execute(text(sql_script))
            messagebox.showinfo('Success', 'Data moved to cleaned tables successfully.')
            connection.commit()
        except Exception as e:
            messagebox.showerror('Error', f'An error occurred: {e}')

def main():
    #establishing connection to the database
    engine = database_connection()
    with engine.connect() as connection:
        messagebox.showinfo('Connection', 'Database connection successful.')
        while True:
            #file selection
            file_path = file_select()         
            gui.create_selection_window()
            year= gui.final_selection.get('year')
            quarter= gui.final_selection.get('quarter')
            if not year or not quarter:
                messagebox.showerror('Error', 'Year or quarter not selected. Exiting program.')
                sys.exit()
            #loading data from csv file to the database
            load_data(file_path, connection, year, quarter)
            if gui.final_selection.get('action') == 'finish':
                break
        #moving data from raw tables to cleaned tables
        move_data_to_cleaned_tables(connection)
if __name__ == '__main__':
    main()